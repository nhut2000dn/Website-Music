<?php
  ...
  if ($check_Insert == true) {
    ...
    if ((!file_exists($_FILES['fileImage']['tmp_name']) || !is_uploaded_file($_FILES['fileImage']['tmp_name'])) && (!file_exists($_FILES['fileAudio']['tmp_name']) || !is_uploaded_file($_FILES['fileAudio']['tmp_name'])))  {
      echo 'You have choose image, audio of song';
    } else {
      $queryInsert = 'INSERT INTO song (id, name, url_name, image, audio, views, dowloads, artist_id, account_id, country_id) VALUES 
        (NULL, "' . $_POST['nameSong'] . '", "' . $urlNameSong . '", "' . $_FILES['fileImage']['name'] . '", "' . $_FILES['fileAudio']['name'] . '", "0", "0", "' . $_POST['idArtist'] . '", "' . $_POST['idAccount'] . '", "' . $_POST['idCountry'] . '");';
