<?php
  ...
  if ($check_update == true) {
    ...
    if (!file_exists($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name']))  {
      $queryUpdate = 'UPDATE account SET full_name="' . $_POST['fullName'] . '", user_types_id="' . $_POST['userType'] . '"  WHERE id=' . $_POST['idAccount'];
      $result   = mysqli_query($connection,$queryUpdate)or die("loi cap nhat".mysqli_error($con));
      if($result) {
        echo 'succesful';
      }
    } else {
      $queryUpdate = 'UPDATE account SET full_name="' . $_POST['fullName'] . '", user_types_id="' . $_POST['userType'] . '", image="' . $_FILES['file']['name'] . '"  WHERE id=' . $_POST['idAccount'];
      $result   = mysqli_query($connection,$queryUpdate)or die("loi cap nhat".mysqli_error($con));
      if($result) {
