...
function RefeshGetUrl() {
  ...
  //check url idCountry
  idCountry = url.searchParams.get("idCountry");
  if (!idCountry) {
...
$.ajax({
  ...
  success:function(response) {
    ...
    $('.title-play-music').html('\
      <span class="title-left">Top 100 song of ' + response['name_top100'] + '</span>\
    ');
    $('.box-img-music').html('\
      <img src="images/country/' + response['image_top100'] + '" alt="">\
    ');
  }
});

// function load top100 songs and play song
function loadPlaylistSong ($numberic) {
  ...
  $.ajax({
    ...
    success:function(response) {
      ...
      var cssSongPlay = '';
      var btnListenSong = '';
      console.log(response['arrayTop100Song']);
      for(var k in response['arrayTop100Song']) {
        if (i == $numberic) {
          idSongCurrent = response['arrayTop100Song'][k]['id_song'];
          btnListenSong = '<i class="fas fa-pause"></i>';
          $('.box-img-small').html('<img src="images/songs/' + response['arrayTop100Song'][k]['image_song'] + '" alt="">');
          $('.name-song').html('Song: '+ response['arrayTop100Song'][k]['name_song']);
          $('.name-arists').html('Artist:'+ response['arrayTop100Song'][k]['name_artist']);
          $('#myAudio').attr('src', 'audio/' + response['arrayTop100Song'][k]['audio_song']);
        ...
        $('.box-playlist').append('\
          <div class="box-item-playlist" ' + cssSongPlay + '>\
            ...
            <div class="txt-playlist">\
              <a href="play-music.html?idSong=' + response['arrayTop100Song'][k]['id_song'] + '" class="link-song">' + response['arrayTop100Song'][k]['name_song'] + ' <span class="txt-artist-playlist"></a>\
               - <a href="play-music.html?idSong=' + response['arrayTop100Song'][k]['id_song'] + '" class="link-artist">' + response['arrayTop100Song'][k]['name_artist'] + '</a></span>\
            </div>\
            <div class="views-playlist-song"><i class="fas fa-headphones-alt"></i> ' + response['arrayTop100Song'][k]['views_song'] + '</div>\
            <div class="btn-add-to-playlist" data-id="' + response['arrayTop100Song'][k]['id_song'] + '">\
              <i class="far fa-heart"></i>\
            </div>\
            <div class="btn-dowloads-song">\
              <a href="audio/' + response['arrayTop100Song'][k]['audio_song'] + '" download="' + response['arrayTop100Song'][k]['name_song'] + '.mp3" class="link-dowload">\
